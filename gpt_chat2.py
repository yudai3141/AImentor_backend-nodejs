...
def weekly_first_system_message_content():
    return f"""
小目標： 簡単なアプリを作る
目標アクション：
    ユーザー認証: データベース登録
...
・肯定的な相槌を交える。ではを使わない
・一度に一つだけ質問を行う
・原因や理由・状態・次に取り組むこと・サポートについて問わない
以下はコーチングでの必須質問です：
・仮に何を行えば達成できたか
プロセスは以下の手順で行います：
1. このステップの目的を話す
...
def weekly_second_system_message_content():
    return f"""
...
def weekly_forth_system_message_content():
    return f"""
小目標： 簡単なアプリを作る
未完了アクション：ユーザー認証: バリデーション、ユーザー認証: データベース登録、ユーザー認証: 更新、データベース連携: セッションの登録、チャット画面: セッション画面の構築、チャット画面: ユーザー画面の構築
