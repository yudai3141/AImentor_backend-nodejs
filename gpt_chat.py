...
def main():
    ...
    if stage == 1:
        ...
        chat_messages = session_messages + [SystemMessage(content=third_system_message_content(sub_goals[goal_num - 1]["subGoal"]))]
        try:
        ...
        try:
            chat_messages = session_messages + [SystemMessage(content=third_system_message_content(sub_goals[goal_num - 1]["subGoal"]))] + user_messages
            chat_messages += user_messages
                ...
                try:
                    completion3 = client.beta.chat.completions.parse(
                        ...
                        messages=[
                            ...
                            {chat_messages}
                            上記の会話ログから、小目標、KPI、numerical_or_TF、KPI_indicator、分解タスクをリスト形式で出力してください。
                            KPI_indicatorは新たに分解したタスクの数としてください。
                            """},
                        ],
                        response_format=ShortTermGoalItemCompleted,
                    )
